...
build-envoy-asan:
	...
	@CC=clang-9 CXX=clang++-9 bazel build --config=clang-asan //src/envoy:envoy
	@cp -f bazel-bin/src/envoy/envoy bin/

build-envoy-tsan:
	@echo "--> building envoy (compilation_mode=fastbuild)"
	@CC=clang-9 CXX=clang++-9 bazel build --config=clang-tsan  //src/envoy:envoy
	@cp -f bazel-bin/src/envoy/envoy bin/


build-envoy:
	@echo "--> building envoy (compilation_mode=release)"
	@CC=clang-9 CXX=clang++-9 bazel build --config=clang-release //src/envoy:envoy
	@cp -f bazel-bin/src/envoy/envoy bin/

build-envoy-debug:
	@echo "--> building envoy (compilation_mode=debug)"
	@CC=clang-9 CXX=clang++-9 bazel build --config=debug //src/envoy:envoy
	@cp -f bazel-bin/src/envoy/envoy bin/
...
test-envoy: clang-format
	...
	@CC=clang-9 CXX=clang++-9  bazel test //src/...

test-envoy-asan: clang-format
	@echo "--> running envoy's unit tests (asan)"
	@CC=clang-9 CXX=clang++-9 ASAN_SYMBOLIZER_PATH=$(which llvm-symbolizer-8) bazel test --config=clang-asan  --test_output=errors //src/...

test-envoy-tsan: clang-format
	@echo "--> running envoy's unit tests (tsan)"
	@CC=clang-9 CXX=clang++-9 ASAN_SYMBOLIZER_PATH=$(which llvm-symbolizer-8) bazel test --config=clang-tsan  --test_output=errors  //src/...
...
clang-format:
	@echo "--> formatting code with 'clang-format-9' tool"
	@echo $(CPP_PROTO_FILES) | xargs clang-format-9 -i
