...
func (s *ServiceInfo) processHttpRule() error {
	...
	if s.Options.Healthz != "" {
		// FIXME: Health check should not associated with any API. But for consistency, just use the first API Name.
		methodName := fmt.Sprintf("%s.%s_HealthCheck", s.ApiNames[0], util.AutogeneratedOperationPrefix)
...
func (s *ServiceInfo) addOptionMethod(apiName string, path string, backendInfo *backendInfo) {
	...
	corsOperationBase := fmt.Sprintf("%s_CORS", util.AutogeneratedOperationPrefix)
