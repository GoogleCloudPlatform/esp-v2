...
func MakeRouteTable(serviceInfo *configinfo.ServiceInfo) ([]*routepb.Route, []*routepb.Route, error) {
	...
	for _, httpPatternMethod := range *httpPatternMethods {
		...
		var err error
		if routeMatchers, methodNotAllowedRouteMatchers, err = makeHttpRouteMatchers(httpRule, seenUriTemplatesInRoute, serviceInfo.Options.AllowColonInWildcardPathSegment); err != nil {
...
func makeHttpRouteMatchers(httpRule *httppattern.Pattern, seenUriTemplatesInRoute map[string]bool, allowColonInWildcardPathSegment bool) ([]*routepb.RouteMatch, []*routepb.RouteMatch, error) {
	if httpRule == nil {
			...
			RouteMatch: &routepb.RouteMatch{
				PathSpecifier: &routepb.RouteMatch_SafeRegex{
					SafeRegex: &matcher.RegexMatcher{
						...
						Regex: httpRule.UriTemplate.Regex(allowColonInWildcardPathSegment),
					},
				},
			},
			UriTemplate: httpRule.UriTemplate.Regex(allowColonInWildcardPathSegment),
		})
