...
import (
	...
	brpb "github.com/GoogleCloudPlatform/api-proxy/src/go/proto/api/envoy/http/backend_routing"
...
func MakeListener(serviceInfo *sc.ServiceInfo) (*v2pb.Listener, error) {
	...
	if serviceInfo.Options.CorsPreset == "basic" || serviceInfo.Options.CorsPreset == "cors_with_regex" {
		...
		glog.V(1).Infof("adding CORS Filter config: %v", corsFilter)
	}
	...
	glog.V(1).Infof("adding Http Connection Manager config: %v", httpConMgr)
