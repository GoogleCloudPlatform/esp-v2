...
type SuccessOPTestCase struct {
	...
	FactoryParamsIn filtergentemp.FactoryParams
...
func (tc *SuccessOPTestCase) RunTest(t *testing.T, factory filtergentemp.FilterGeneratorOPFactory) {
	...
	t.Run(tc.Desc, func(t *testing.T) {
		...
		for i, gotGenerator := range gotGenerators {
			...
			gotHTTPFilter, err := filtergentemp.FilterConfigToHTTPFilter(gotConfig, gotGenerator.FilterName())
			if err != nil {
...
// RunTest is a test helper to run the test.
func (tc *FactoryErrorOPTestCase) RunTest(t *testing.T, factory filtergentemp.FilterGeneratorOPFactory) {
	t.Helper()
	t.Run(tc.Desc, func(t *testing.T) {
		opts := options.DefaultConfigGeneratorOptions()
...
// RunTest is a test helper to run the test.
func (tc *GenConfigErrorOPTestCase) RunTest(t *testing.T, factory filtergentemp.FilterGeneratorOPFactory) {
	t.Helper()
	t.Run(tc.Desc, func(t *testing.T) {
		opts := options.DefaultConfigGeneratorOptions()
		if err := mergo.Merge(&opts, tc.OptsIn, mergo.WithOverride); err != nil {
