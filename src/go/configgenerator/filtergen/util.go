...
func MethodToCORSSelector(api *apipb.Api, method *apipb.Method, corsUtility CorsUtility, opts options.ConfigGeneratorOptions) string {
	return api.GetName() + corsUtility.corsOperationDelimiter + method.GetName()
}

// CORSSelectorToSelector reverses MethodToCORSSelector to extract the original
// selector name from a CORS selector.
func CORSSelectorToSelector(corsSelector string, corsUtility CorsUtility) (string, error) {
	if !strings.Contains(corsSelector, corsUtility.corsOperationDelimiter) {
		// Not a CORS selector, no op.
		return "", nil
	}

	split := strings.Split(corsSelector, corsUtility.corsOperationDelimiter)
	if len(split) != 2 {
