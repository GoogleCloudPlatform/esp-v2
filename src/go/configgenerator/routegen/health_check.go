	...
	return []RouteGenerator{
		&HealthCheckGenerator{
			...
			BackendRouteGen: &helpers.BackendRouteGenerator{
				...
				RetryCfg:                           helpers.NewRouteRetryConfigerFromOPConfig(opts),
				HSTSCfg:                            helpers.NewRouteHSTSConfigerFromOPConfig(opts),
				OperationNameCfg:                   helpers.NewRouteOperationNameConfigerFromOPConfig(opts),
				DeadlineCfg:                        helpers.NewRouteDeadlineConfigerFromOPConfig(opts),
			},
		},
	}, nil
}
...
func (g *HealthCheckGenerator) GenRouteConfig() ([]*routepb.Route, error) {
	...
	methodName := fmt.Sprintf("%s_HealthCheck", g.AutogeneratedOperationPrefix)
	operationName := fmt.Sprintf("%s.%s", g.ESPOperationAPI, methodName)
