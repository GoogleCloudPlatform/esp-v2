...
var (
  ...
  omitted, the proxy contacts the metadata service to fetch an access token`)
	LocalAccessTokenServerPort = flag.Uint("local_access_token_server_port", 8791, "Port that configmanager use to setup server to provide envoy with access token using service account credential, for accessing servicecontrol.")
...
func EnvoyConfigOptionsFromFlags() options.ConfigGeneratorOptions {
	opts := options.ConfigGeneratorOptions{
		...
		LocalAccessTokenServerPort:              *LocalAccessTokenServerPort,
