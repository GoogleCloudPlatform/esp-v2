...
var (
  ...
  omitted, the proxy contacts the metadata service to fetch an access token`)
	...
	SmCallRetryConfigs       = flag.String("service_management_call_retry_configs", "", `Set retry times and retry intervals for different status calling service management. For example, setting retry 30 times with 3sec interval for status 429 is {"429":{"RetryNum":30,"RetryInterval":3000000000}}. This flag is used for test only`)
