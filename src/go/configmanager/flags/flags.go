...
var (
  ...
  When disabled, config generator will not make external calls to determine the JWKS URI, 
	...
	JwksFetchRetryBackOffBaseInterval = flag.Duration("jwks_fetch_retry_back_off_base_interval", 200*time.Millisecond, `Specify JWKS fetch retry exponential back off base interval in milliseconds. The default is 200 milliseconds.`)
	JwksFetchRetryBackOffMaxInterval  = flag.Duration("jwks_fetch_retry_back_off_max_interval", 32*time.Second, `Specify JWKS fetch retry exponential back off maximum interval in milliseconds. The default is 32 seconds.`)
