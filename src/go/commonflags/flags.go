...
var (
	...
	TracingProjectId                = flag.String("tracing_project_id", defaults.TracingOptions.ProjectId, "DEPRECATED. Sets the originating GCP Project ID. Please use standard OTel environment variables instead (e.g., OTEL_RESOURCE_ATTRIBUTES).")
	TracingStackdriverAddress       = flag.String("tracing_stackdriver_address", defaults.TracingOptions.StackdriverAddress, "DEPRECATED. Overrides the telemetry exporter destination. Please use standard OTel environment variables instead (e.g., OTEL_EXPORTER_OTLP_ENDPOINT).")
	...
	TracingMaxNumAttributes         = flag.Int64("tracing_max_num_attributes", defaults.TracingOptions.MaxNumAttributes, "DEPRECATED. Sets the maximum number of attributes that each span can contain. Defaults to the maximum allowed by Stackdriver. In practice, the number of attributes published will be much less.")
	TracingMaxNumAnnotations        = flag.Int64("tracing_max_num_annotations", defaults.TracingOptions.MaxNumAnnotations, "DEPRECATED. Sets the maximum number of annotations that each span can contain. Defaults to the maximum allowed by Stackdriver. In practice, the number of annotations published will be much less.")
	TracingMaxNumMessageEvents      = flag.Int64("tracing_max_num_message_events", defaults.TracingOptions.MaxNumMessageEvents, "DEPRECATED. Sets the maximum number of message events that each span can contain. Defaults to the maximum allowed by Stackdriver. In practice, the number of message events published will be much less.")
	TracingMaxNumLinks              = flag.Int64("tracing_max_num_links", defaults.TracingOptions.MaxNumLinks, "DEPRECATED. Sets the maximum number of links that each span can contain. Defaults to the maximum allowed by Stackdriver. In practice, the number of links published will be much less.")
