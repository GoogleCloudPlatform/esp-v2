...
Status LogsMetricsLoader::Load(const Service& service,
  ...
  LogsMetricsLoader lml(RequestBuilder::IsLabelSupported,
                        RequestBuilder::IsMetricSupported);
...
Status LogsMetricsLoader::AddLoggingDestinations(
  ...
  for (int d = 0, dsize = destinations.size(); d < dsize; d++) {
    ...
    // Store names of logs API Proxy should log into.
...
Status LogsMetricsLoader::LoadLogsMetrics(const Service& service,
  ...
  // API Proxy logs into all producer destination logs.
  ...
  // API Proxy reports producer and consumer metrics.
