...
func TestServiceControlCheckError(t *testing.T) {
	...
	testData := []struct {
		...
		{
			desc:    "Failed, the service control return API_KEY_INVALID and no consumer project id so no byConsumer metrics",
		...
		{
			desc:   "Failed, auth passed but check failed with PROJECT_INVALID",
