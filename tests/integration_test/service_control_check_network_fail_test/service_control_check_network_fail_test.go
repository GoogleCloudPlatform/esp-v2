...
func TestServiceControlCheckNetworkFail(t *testing.T) {
	...
	testdata := []struct {
		...
		},
		{
			...
			allocatedPort:     platform.TestServiceControlCheckWrongServerName,
			wantError:         `503 Service Unavailable, {"code":503,"message":"UNAVAILABLE: Calling Google Service Control API failed with: 503 and body: upstream connect error or disconnect/reset before headers. reset reason: remote connection failure, transport failure reason: delayed connect error:`,
		},
	}
