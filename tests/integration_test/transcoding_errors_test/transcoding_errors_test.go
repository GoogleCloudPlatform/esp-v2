...
func TestTranscodingBackendUnavailableError(t *testing.T) {
	...
	tc := TranscodingTestType{
		...
		wantErr:        `503 Service Unavailable, {"code":503,"message":"upstream connect error or disconnect/reset before headers. retried and the latest reset reason: remote connection failure, transport failure reason: delayed connect error:`,
	}
