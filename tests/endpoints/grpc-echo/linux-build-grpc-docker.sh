...
GRPC_ROOT="${ROOT}/tests/grpc"

. "${ROOT}/tests/e2e/scripts/prow-utilities.sh" || { echo "Cannot load Bash utilities"; exit 1; }

TEST_SERVER_TARGET='//tests/grpc:grpc-test-server'
TEST_SERVER_BIN='tests/grpc/grpc-test-server'
...
if ! [[ -e "${BAZEL_TARGET}" ]]; then
  ...
  bazel build --config=release "${TEST_SERVER_TARGET}" \
    p| error_exit 'Could not build ${TEST_SERVER_BIN}'
fi
